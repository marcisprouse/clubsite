...
class Event(models.Model):
    ...
    exclude = models.BooleanField(_("exclude from list"), default=False, help_text="Check if you do NOT want this event to show up on the activity list page (it will still show up on the calendar).")
...
class Occurrence(models.Model):
    ...
    need_volunteers = models.BooleanField(_("need volunteers"), default=False, help_text="Check if you would like to have volunteers for this occassion.")
    volunteers = models.ManyToManyField(django_settings.AUTH_USER_MODEL,
        through='Volunteer', related_name='volunteers_owned')
    memberrsvps = models.ManyToManyField(django_settings.AUTH_USER_MODEL,
        through='MemberRsvp', related_name='memberrsvps_owned')
    bring = models.BooleanField(_("member brings dish or item"), default=False, help_text="Check if you would like for members to sign up to bring a dish or item.  Be sure to have the details of dish or item ideas in your description.")
    rsvp = models.BooleanField(_("member rsvp"), default=False, help_text="Check if you would like for members to let you know they are coming.")
    exclude = models.BooleanField(_("exclude from list"), default=False, help_text="Check if you do NOT want this event to show up on the activity list page (it will still show up on the calendar).")
    ...
    def __str__(self):
        return gettext("%(title)s -- %(start)s to %(end)s") % {
            "title": self.title,
...
class Volunteer(models.Model) :
    text = models.TextField(
        validators=[MinLengthValidator(3, "Volunteer comment must be greater than 3 characters")]
    )
...
class Memberrsvp(models.Model) :
    ...
    mealchoice = models.CharField(max_length=30,
                              choices=CHOICES,
                              default='------', help_text = "Which item will you bring for fundraising silent auction?")

    bringing = models.TextField(default="", blank=True, null=True, help_text='If you are attending, please indicate what you may bring based on the request in the description of this event (i.e., dish, item for yard sale, etc.)'
    )

    guests = models.TextField(default="", blank=True, null=True, help_text='Please indicate in this box the names of guests you plan to bring and whether or not they are club members or OUTSIDE of Coyote Lakes neighborhood.')
    ...
    # Shows up in the admin list
    def __str__(self):
        return u"%s,%s ------ %s --%s ------ %s" % (self.owner.last_name, self.owner.first_name, self.attending, self.occurrence.title, self.bringing )
